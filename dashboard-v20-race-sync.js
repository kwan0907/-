...
window.__SB_GLOBAL_RACE_SYNC__='20260916-rs1';
var wanted=null,source='',syncing=false,frameSeen=new WeakSet(),scheduled=0,frameHandlers=new WeakMap();
function valid(v){var n=Number(v);return Number.isInteger(n)&&n>=1&&n<=12?n:null}
function selected(box){if(!box)return null;var b=box.querySelector('[data-race].on,[data-race].active,[data-race][aria-pressed="true"]');return b?valid(b.getAttribute('data-race')):null}
...
function syncParent(n){var box=document.getElementById('races');var b=button(box,n);if(!b)return false;if(selected(box)!==n){b.click();return false}return true}
function syncFrame(n){var f=info(frame());if(!f)return false;var b=button(f.d.getElementById('raceChips'),n);if(!b)return false;var state=f.w.state;var current=state&&valid(state.raceNo);if(current!==n){b.click();return false}return true}
...
function queue(n,from){n=valid(n);if(!n)return;wanted=n;source=from;if(!scheduled)scheduled=setTimeout(sync,0)}
