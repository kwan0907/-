(function(){
'use strict';
if(window.__SB_SIGNAL_LABELS__)return;window.__SB_SIGNAL_LABELS__='20260916-label3';
var d=document;
...
function fixRows(){var root=d.getElementById('anomalyTop5');if(!root)return;replaceText(root);root.querySelectorAll('.dashrow').forEach(function(r){var p=r.querySelector('.statuspill'),sc=scoreOf(r),rk=rankOf(r),z=label(sc,rk);if(p&&z){if(p.textContent!==z[0])p.textContent=z[0];p.classList.remove('hot','cold','bad','sb-xstrong','sb-strong','sb-weak','sb-xweak','sb-normal');p.classList.add(z[1])}if(!r.classList.contains('sb-labelled'))r.classList.add('sb-labelled')})}
...
style();run();setTimeout(run,300);setInterval(run,1200);
})();
